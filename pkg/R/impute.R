...
impute <- function(vcf=NULL, vcf_RData=NULL,
                   ...
                   ){
  {
    ...
    for(chromo in chromo_set){
      ...
      if(log != FALSE){
        ...
        cat("Copyright (C) 2019-2020 Torsten Pook\n\n")
      }
      ...
      if(length(vcf)>0){
        ...
        for(subindex in 2:ncol(data@gt)){
          print(subindex)

          subdata <- strsplit(data@gt[take,subindex], ":", fixed=TRUE)
          lname <- length(name)
      ...
      if(activ_HB){
        ...
        if(hetero){
          ...
          new_depth <- hb_depth <-  estimated_cnv <- estimated_deletion <- matrix(0L, nrow=nrow(haplo), ncol=ncol(geno)*2)
        } else{
          geno_imputed <- matrix(NA, nrow=nrow(geno), ncol=ncol(geno))
          new_depth <- hb_depth <-  estimated_cnv <- estimated_deletion <- matrix(0L, nrow=nrow(geno), ncol=ncol(geno))
        }
        ...
        if(hetero){
          ...
          rmax <- RandomFieldsUtils::colMax(t(geno))
        }
        ...
        if(overwrite_na & !hetero){
          to_replace <- is.na(geno_imputed) & !is.na(geno) & depth >= overwrite_na_min_depth
          geno_imputed[to_replace] <- geno[to_replace]
      ...
      if(length(ref_panel)>0 && FALSE){
        beagle_commandline <- paste0("java -jar ", path_beaglejar," ne=", beagle_ne," gt=",path_prebeagle2," out=",out," nthreads=", beagle_core," ref=", ref_panel, " impute=false")
        system(beagle_commandline)
      } else{
